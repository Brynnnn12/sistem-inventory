...
<head>
    <meta charset="UTF-8">
    <title>Surat Jalan - {{ $outbound->code }}</title>
    <style>
        @page { margin: 0.4cm; }
        body {
...
<body>
    ...
    <div class="doc-title">SURAT JALAN BARANG KELUAR</div>

    <div class="doc-info">
        <table>
            <tr><td class="label">No. Bukti</td><td>: <strong>{{ $outbound->code }}</strong></td></tr>
            <tr><td class="label">Tanggal</td><td>: {{ \Carbon\Carbon::parse($outbound->sale_date)->isoFormat('D MMMM YYYY') }}</td></tr>
            <tr><td class="label">Gudang</td><td>: {{ $outbound->warehouse->name }}</td></tr>
        </table>
    </div>

    <div class="party-section">
        <div class="title">KEPADA YTH</div>
        <div class="name">{{ $outbound->customer->name }}</div>
        @if($outbound->customer->address)
            <div class="detail">{{ $outbound->customer->address }}</div>
        @endif
        <div class="detail">
            Telp: {{ $outbound->customer->phone ?? '-' }}
            @if($outbound->customer->contact_person) | Kontak: {{ $outbound->customer->contact_person }} @endif
        </div>
    </div>
    ...
    <table class="items">
        ...
        <tbody>
            <tr>
                ...
                <td class="text-center">{{ $outbound->product->unit }}</td>
                <td class="text-right">{{ number_format($outbound->quantity, 0, ',', '.') }}</td>
                <td class="text-right">Rp {{ number_format($outbound->unit_price, 0, ',', '.') }}</td>
                <td class="text-right">Rp {{ number_format($outbound->quantity * $outbound->unit_price, 0, ',', '.') }}</td>
            </tr>
            <tr class="total-row">
    ...
    <div class="footer">
        {{ config('app.name') }} — {{ \Carbon\Carbon::now()->isoFormat('D MMMM YYYY HH:mm') }} | {{ $outbound->creator->name }}
    </div>
</body>
</html>
